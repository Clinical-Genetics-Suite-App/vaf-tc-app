...
本アプリはTCに基づいて4つの状況依存アラートを生成します。TCの閾値（≤ 20%・≥ 60%）は *Journal of Human Genetics* 掲載論文における本ソフトウェアの記述に準拠しています。TC ≥ 90%のアラートは論文に記載のない追加機能です。
...
TCの上昇に伴い somatic (LOH with Del) は germline (Hetero) の50%を上回り（TC = 2/3、約66.7%）、生殖細胞系列のLOH線に接近します。以下の条件を両方満たす場合に発動します：
...
- **4つのアラート**：TCの値に基づく状況依存アラート（≤ 20% / ≥ 60% / ≥ 90%）
...
| ファイル | 説明 |
|------|-------------|
| app.py | メインStreamlitアプリ・英語版(ver 3.6) |
| app_ja.py | メインStreamlitアプリ・日本語版(ver 3.6) |
...
- **修正** LOH収束アラートの閾値を **TC ≥ 70% → ≥ 60%** に変更。ver 3.5 は公開前の校正刷り（「≥70%」と記載）に基づいて実装していたが、公開版は「≥60%」であったため訂正。これによりFig. 1 の TC ≥ 60% 網掛け帯と閾値が一致する。
- **明記** TC ≥ 90% のアラートは論文に記載のない追加機能である旨を注記。
...
- **変更** アラート閾値を論文の記述に合わせ **TC ≤ 20% / ≥ 60%** に統一。従来のグレーゾーン(TC 61〜66%)はTC ≥ 60%アラートと論点が重複するため廃止
